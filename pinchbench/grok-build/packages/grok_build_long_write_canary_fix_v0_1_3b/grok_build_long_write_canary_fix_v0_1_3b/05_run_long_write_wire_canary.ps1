...
$ActualBytes = [System.IO.File]::ReadAllBytes($Probe)
$ActualHash = [System.BitConverter]::ToString(
    [System.Security.Cryptography.SHA256]::Create().ComputeHash($ActualBytes)
).Replace("-","").ToLowerInvariant()

$PayloadWithoutTerminalLf = $Payload.Substring(0,$Payload.Length-1)
$ExpectedWithoutTerminalLfBytes = $Utf8NoBom.GetBytes($PayloadWithoutTerminalLf)
$ExpectedWithoutTerminalLfHash = [System.BitConverter]::ToString(
    [System.Security.Cryptography.SHA256]::Create().ComputeHash(
        $ExpectedWithoutTerminalLfBytes
    )
).Replace("-","").ToLowerInvariant()

$ExactPayloadMatch = $ActualHash -eq $ExpectedHash
$TerminalLfNormalizedMatch =
    $ActualHash -eq $ExpectedWithoutTerminalLfHash -and
    $ActualBytes.Length -eq $ExpectedWithoutTerminalLfBytes.Length

if (-not $ExactPayloadMatch -and -not $TerminalLfNormalizedMatch) {
    throw "Long-write file mismatch beyond the permitted terminal-LF normalization. ExpectedSHA256=$ExpectedHash ExpectedWithoutTerminalLfSHA256=$ExpectedWithoutTerminalLfHash ActualSHA256=$ActualHash ActualBytes=$($ActualBytes.Length)"
}

if ($TerminalLfNormalizedMatch) {
    Write-Host "INFO: Grok write tool normalized away exactly one terminal LF; all payload bytes before it match."
} else {
    Write-Host "PASS: long-write file exactly matches payload including terminal LF."
}
...
Write-Host ""
Write-Host "PASS: long-write tool result completed through adapter v0.1.3; content matched with at most one terminal-LF normalization." -ForegroundColor Green
