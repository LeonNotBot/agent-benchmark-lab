...
2. OpenRouter streamed item type "openrouter:web_search", while Grok Build expects
   "web_search_call".
3. Some requests explicitly disabled reasoning although the selected endpoint requires it.
...
def normalize_response_text(text: str) -> tuple[str, bool]:
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": "PinchBench-GrokBuild-Search-Adapter/0.1.0",
            ...
            if is_sse:
                ...
                for raw_line in upstream:
                    ...
                    if line.startswith("data:"):
                        ...
                        normalized, changed = normalize_response_text(payload_text)
                        if changed:
                            translated_count += 1
                            self.server.logger.write(
                                "response_event_translated",
                                request_id=request_id,
                                from_type="openrouter:web_search",
                                to_type="web_search_call",
                            )
            ...
            normalized_text, changed = normalize_response_text(response_text)
            ...
            self.server.logger.write(
                ...
                status=status,
                stream=False,
                translated_count=1 if changed else 0,
            )
