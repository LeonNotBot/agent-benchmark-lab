#!/usr/bin/env python3
# Runner revision: 2026-07-28-pinchbench-gemini-cli-windows-v1-adapter-1.2
r"""
Run the local PinchBench task set with Gemini CLI on native Windows.

Target evaluation chain:
    PinchBench local tasks -> Gemini CLI built-in agent/tools -> OpenRouter
...
- Same JSON/CSV/XLSX result structure, with extra Gemini audit fields.
- Fresh project-scoped Gemini session for normal tasks; explicit --resume for multi-session turns.
- Raw stdout JSONL and stderr are retained separately for audit.

Intentional agent-adapter differences:
- Gemini CLI is pinned separately and invoked in headless stream-json mode.
- An isolated GEMINI_CLI_HOME and explicit custom-gateway settings prevent user/project customizations.
- --approval-mode yolo is required for unattended Gemini tool execution; it is recorded explicitly.
- Gemini does not expose OpenCode's reasoning-token/cost fields in the same shape, so those
  fields remain null when unavailable.

Recommended location:
    C:\pinchbench-gemini\runner\run_pinchbench_gemini_windows.py

Examples:
    python run_pinchbench_gemini_windows.py --preflight --suite all
    python run_pinchbench_gemini_windows.py --suite task_sanity,task_iterative_code_refine --keep-workspaces --verbose
    python run_pinchbench_gemini_windows.py --suite all --keep-workspaces --verbose

Scoring:
- Automated tasks use PinchBench's checked-out automated grading code.
- Hybrid and llm_judge tasks use the same checked-out PinchBench grading code,
  with only --judge-model overridden through judge_backend="api".
- The judge is separate from the tested Gemini CLI + DeepSeek V4 Pro agent.
"""
...
os.environ.setdefault("GEMINI_CLI_TRUST_WORKSPACE", "true")
...
try:
    ...
    raise SystemExit(
        ...
        r"C:\pinchbench-gemini\.venv\Scripts\python.exe -m pip install pyyaml"
    ) from exc

LOGGER = logging.getLogger("pinchbench-gemini-cli")

RUNNER_REVISION = "2026-07-31-gemini-cli-windows-v1-adapter-1.3-web-bridge"
DEFAULT_MODEL = "deepseek/deepseek-v4-pro"
DEFAULT_GEMINI_VERSION = "0.52.0"
DEFAULT_ADAPTER_VERSION = "1.3.0"
DEFAULT_ADAPTER_URL = "http://127.0.0.1:8766"
...
SKIP_WORKSPACE_NAMES = {
    ...
    "TOOLS.md", "AGENTS.md", "CLAUDE.md", "GEMINI.md",
}
SKIP_WORKSPACE_DIRS = {
    ".git", ".openclaw", ".opencode", ".gemini", "__pycache__", "node_modules", "skills",
}
...
def choose_gemini_command(preferred: str, inferred_root: Optional[Path] = None) -> str:
    if preferred and preferred != "auto":
        ...
        if resolved:
            return resolved
        raise SystemExit(f"找不到 Gemini CLI 命令 {preferred!r}")

    candidates: list[Path] = []
    if inferred_root is not None:
        candidates.extend([
            inferred_root / "cli" / "node_modules" / ".bin" / "gemini.cmd",
            inferred_root / "cli" / "node_modules" / ".bin" / "gemini.exe",
        ])
    for candidate in candidates:
        if candidate.exists():
            return str(candidate.resolve())

    for command in (("gemini.cmd", "gemini.exe", "gemini") if sys.platform == "win32" else ("gemini",)):
        resolved = shutil.which(command)
        if resolved:
            return resolved

    raise SystemExit(
        "找不到 Gemini CLI。建议固定安装到 "
        r"C:\pinchbench-gemini\cli\node_modules\.bin\gemini.cmd，"
        "并通过 --gemini-cli 指定。"
    )
...
def extract_gemini_error(event: dict[str, Any]) -> str:
    ...
    if event_type == "result":
        ...
        if error:
            return str(error)
        return str(event.get("message") or status)
    if event_type == "error":
        error = event.get("error")
        if isinstance(error, dict):
            return str(error.get("message") or json.dumps(error, ensure_ascii=False))
        return str(error or event.get("message") or "Gemini CLI emitted an error")
    return ""


def parse_gemini_result(event: dict[str, Any]) -> Optional[dict[str, Any]]:
    ...
    return {
        ...
        "output_tokens": _to_int(stats.get("output_tokens")),
        "cache_read_tokens": _to_int(
            stats.get("cached")
            or stats.get("cache_read_tokens")
        ),
        "total_tokens": _to_int(stats.get("total_tokens")),
        "duration_ms": _to_int(stats.get("duration_ms")),
        "tool_calls": _to_int(stats.get("tool_calls")),
        "models": stats.get("models") if isinstance(stats.get("models"), dict) else {},
    }


def normalize_gemini_event(event: dict[str, Any]) -> list[dict[str, Any]]:
    """Convert Gemini stream-json events to PinchBench transcript shape."""
    event_type = str(event.get("type") or "")
    ...
    if event_type == "message":
        ...
        if role in CLIENT_VISIBLE_ROLES and isinstance(content, str) and content:
            normalized.append({
                "type": "message",
                "message": {
                    "role": "assistant",
    ...
    if event_type == "tool_use":
        ...
        normalized.append({
            ...
            "message": {
                ...
                "content": [{
                    "type": "toolCall",
                    "id": str(event.get("tool_id") or ""),
                    "name": str(event.get("tool_name") or ""),
                    "arguments": parameters,
                    "input": parameters,
                }],
            },
            "_adapter": {"source": "gemini-cli", "event_type": event_type},
        })
        return normalized
    ...
    if event_type == "tool_result":
        ...
        normalized.append({
            ...
            "message": {
                ...
                "content": [{
                    "type": "toolResult",
                    "toolCallId": str(event.get("tool_id") or ""),
                    "name": str(event.get("tool_name") or ""),
                    "content": _stringify_tool_content(event.get("output")),
                    "isError": status not in {"", "success"},
                }],
            },
            "_adapter": {"source": "gemini-cli", "event_type": event_type},
        })
    ...
    error_text = extract_gemini_error(event)
    if error_text:
        normalized.append({
            "type": "message",
            "message": {
                ...
                "content": [{
                    "type": "text",
                    "text": f"[Gemini CLI error] {error_text}",
                }],
            },
            "_adapter": {"source": "gemini-cli", "event_type": event_type},
        })
...
def make_user_transcript_event(
    prompt: str,
    turn_index: int,
    session_label: str,
) -> dict[str, Any]:
    return {
        ...
        "_adapter": {
            "source": "pinchbench-gemini-cli-runner",
            "turn": turn_index,
...
def run_gemini_streaming(
    ...
    """Run one Gemini CLI headless turn with UTF-8 stdin and stream-json."""
    ...
    env["NO_COLOR"] = "1"
    env["FORCE_COLOR"] = "0"
    # Normalize response/search language without changing the benchmark prompt.
    ...
    env["GOOGLE_GEMINI_BASE_URL"] = adapter_url.rstrip("/")
    env["GOOGLE_GENAI_API_VERSION"] = "v1beta"
    env["GOOGLE_GENAI_USE_VERTEXAI"] = "false"
    # The child needs only the local placeholder. The real OpenRouter key
    # remains in the runner for the Judge and in window A for the adapter.
    ...
    if proc.stdout is None or proc.stderr is None:
        kill_proc_tree(proc)
        raise RuntimeError("Gemini CLI stdout/stderr pipe creation failed")
    ...
    with (
        ...
        while open_streams or proc.poll() is None:
            ...
            if event_type == "init":
                ...
                if candidate_session and not session_id:
                ...
                if tool_status not in {"", "success"}:
                    tool_errors += 1
                    permission_text = _stringify_tool_content(
                        event.get("output")
                        or event.get("error")
                    )
                    if re.search(
                        r"(?i)permission|denied|not approved|approval",
                        permission_text,
                    ):
                        permission_denials.append(permission_text)

            normalized_events.extend(normalize_gemini_event(event))

            parsed_result = parse_gemini_result(event)
            if parsed_result is not None:
                result_seen = True
                result_success = bool(parsed_result["success"])
                usage_seen = True
                ...
                step_count = parsed_result["tool_calls"]
                tool_call_count = max(
                    tool_call_count,
                    parsed_result["tool_calls"],
                )
                for model_name in parsed_result["models"].keys():
                    if model_name:
                        observed_models.add(str(model_name))
                if not result_success:
                    status = "error"

            event_error = extract_gemini_error(event)
            if event_error:
    ...
    if returncode not in (0, None) and status == "success":
        status = "error"
        error_messages.append(stderr or f"Gemini CLI 退出码 {returncode}")

    if returncode in (0, None) and status == "success" and not result_seen:
        status = "error"
        error_messages.append("Gemini CLI 未输出最终 result 事件")
    ...
    return {
        ...
        "output": output,
        "error": " | ".join(
            dict.fromkeys(message for message in error_messages if message)
        ),
        "stderr": stderr,
        "elapsed": elapsed,
        "ttft": ttft,
        ...
        "finish_reasons": ["success"] if result_success else [],
...
def grade_with_pinchbench_default(
    *,
    ...
    """Use the checked-out PinchBench grader with an explicit Judge model."""
...
def save_csv(results: list[dict[str, Any]], output_path: Path) -> None:
    ...
    fields = [
        ...
        "gemini_cli_versions", "permission_modes", "permission_denials",
...
def save_xlsx(
    ...
    headers = [
        ...
        "Gemini版本", "权限模式", "权限拒绝", "Workspace", "Transcript", "运行错误",
        "打分错误", "打分备注",
    ]
    ...
    for row_index, row in enumerate(results, start=2):
        values = [
            ...
            json.dumps(row.get("gemini_cli_versions") or [], ensure_ascii=False),
...
def write_run_config(
    ...
    gemini_version = command_output([gemini_command, "--version"])
    config = {
        ...
        "expected_gemini_cli_version": args.expected_gemini_version,
        "model": args.model,
        "agent": "Gemini CLI built-in default agent/tools",
        "variant": "default; no custom system prompt or tool allow-list",
        ...
        "gemini_cli_home": str(Path(args.gemini_home).expanduser().resolve()),
        "adapter_url": args.adapter_url,
        "adapter_expected_version": args.expected_adapter_version,
        "adapter_health": adapter_health,
        "adapter_chain": (
            "Gemini CLI -> localhost Gemini protocol adapter -> "
            "OpenRouter Responses API -> deepseek/deepseek-v4-pro"
        ),
        "child_openrouter_key_removed": True,
        "worker_count": 1,
        "task_concurrency": 1,
        "skill_dir": str(skill_dir),
        "tasks_dir": str(tasks_dir),
        "pinchbench_commit": commit,
        "expected_pinchbench_commit": args.expected_pinchbench_commit,
        ...
        "judge_backend": "api",
        "judge_model": judge_model,
        "judge_model_source": "--judge-model",
        ...
        "comparability_note": (
            "Task loading, manifest order, four integration exclusions, "
            "timeouts, workspace staging, transcript normalization, official "
            "PinchBench grader, hybrid weights, score normalization, exports "
            "and Judge backend are retained from the validated Qwen Code "
            "Windows runner. Gemini CLI invocation and event parsing are the "
            "only agent-adapter differences."
        ),
    }
...
def read_adapter_health(
    ...
    try:
        ...
        if parsed.scheme != "http" or parsed.hostname not in {
            ...
            return None, (
                "Adapter URL must be localhost HTTP for key isolation: "
                f"{adapter_url}"
            )
        url = adapter_url.rstrip("/") + "/healthz"
        ...
        if not isinstance(payload, dict):
            return None, "Adapter health response is not a JSON object."
        return payload, None
    except Exception as exc:
        return None, f"Adapter health check failed: {exc}"


def find_gemini_context_files(
    inferred_root: Path,
    gemini_home: Path,
    skill_dir: Path,
) -> list[str]:
    candidates = [
        inferred_root / "GEMINI.md",
...
def print_preflight(
    ...
    if not actual_commit:
        gemini_failures.append(
            "Cannot determine PinchBench git commit from skill dir."
        )
    elif (
        args.expected_pinchbench_commit
        and actual_commit != args.expected_pinchbench_commit
    ):
        gemini_failures.append(
            "PinchBench commit mismatch: expected "
            f"{args.expected_pinchbench_commit}, actual {actual_commit}"
        )
    ...
    if (
        ...
        and actual_manifest_sha256.lower()
        != args.expected_manifest_sha256.lower()
    ):
        gemini_failures.append(
            "PinchBench manifest SHA-256 mismatch: expected "
            f"{args.expected_manifest_sha256}, "
            f"actual {actual_manifest_sha256 or 'missing'}"
        )

    gemini_version_output = command_output([gemini_command, "--version"])
    version_match = re.search(
        r"\d+\.\d+\.\d+(?:[-+][^\s]+)?",
        gemini_version_output,
    )
    gemini_version = (
        version_match.group(0)
        if version_match
        else gemini_version_output.strip()
    )
    if (
        args.expected_gemini_version
        and gemini_version != args.expected_gemini_version
    ):
        gemini_failures.append(
            "Gemini CLI 版本不一致：预期 "
            f"{args.expected_gemini_version}，实际 {gemini_version_output}"
        )
    ...
    if not settings_path.exists():
        gemini_failures.append(
            f"找不到 Gemini settings.json: {settings_path}"
        )
    else:
        settings, settings_error = read_json_file(settings_path)
        if settings_error:
            gemini_failures.append(settings_error)
            ...
            if selected_type != "gemini-api-key":
                gemini_failures.append(
                    "settings security.auth.selectedType 必须是 "
                    f"gemini-api-key，实际是 {selected_type!r}"
                )

    context_files = find_gemini_context_files(
        inferred_root,
        gemini_home,
        skill_dir,
    )
    if context_files:
        gemini_failures.append(
            "检测到可能注入自定义指令的 GEMINI.md: "
            + " | ".join(context_files)
        )

    adapter_health, adapter_error = read_adapter_health(args.adapter_url)
    if adapter_error:
        gemini_failures.append(adapter_error)
        adapter_health = {}
    else:
        assert adapter_health is not None
        if adapter_health.get("ok") is not True:
            gemini_failures.append("Adapter health ok != true")
        if adapter_health.get("ready") is not True:
            gemini_failures.append(
                "Adapter is not ready; window A may lack OPENROUTER_API_KEY."
            )
        actual_version = str(adapter_health.get("version") or "")
        if (
            args.expected_adapter_version
            and actual_version != args.expected_adapter_version
        ):
            gemini_failures.append(
                "Adapter version mismatch: expected "
                f"{args.expected_adapter_version}, actual {actual_version}"
            )
        forced_model = str(adapter_health.get("forcedModel") or "")
        if forced_model and forced_model != args.model:
            gemini_failures.append(
                f"Adapter forcedModel={forced_model}, runner model={args.model}"
            )
        response_mode = str(adapter_health.get("responseMode") or "")
        if response_mode and response_mode != "heartbeat_stream":
            gemini_failures.append(
                f"Adapter responseMode is {response_mode}, expected heartbeat_stream"
            )

    print("=" * 100)
    print("PinchBench Gemini CLI Windows runner preflight")
    print("=" * 100)
    print(f"Runner revision      : {RUNNER_REVISION}")
    print(f"Skill dir            : {skill_dir}")
    print(f"Tasks dir            : {tasks_dir}")
    print(f"PinchBench commit    : {actual_commit or 'unavailable'}")
    print(f"Expected commit      : {args.expected_pinchbench_commit}")
    print(f"Manifest SHA-256     : {actual_manifest_sha256 or 'missing'}")
    print(f"Expected manifest    : {args.expected_manifest_sha256}")
    print(f"Gemini command       : {gemini_command}")
    print(f"Gemini version       : {gemini_version_output}")
    print(f"Expected version     : {args.expected_gemini_version}")
    print(f"Gemini home          : {gemini_home}")
    print(f"Settings             : {settings_path}")
    print(f"Model                : {args.model}")
    print(f"Approval mode        : {args.approval_mode}")
    print("Skip trust           : yes")
    print("Sandbox              : no")
    print(f"Adapter URL          : {args.adapter_url}")
    print(
        "Adapter version      : "
        f"{adapter_health.get('version', 'unavailable')}"
    )
    print(
        "Adapter response     : "
        f"{adapter_health.get('responseMode', 'unavailable')}"
    )
    ...
    print("Built-in agent/tools : yes; no custom GEMINI.md")
    print("Prompt transport     : UTF-8 stdin file (never argv)")
    print("Output transport     : Gemini stream-json; stderr separate")
    ...
    print("Adapter key location : window A only")
    print("Judge key location   : runner window B")
    ...
    for key, value in sorted(category_counts.items()):
        print(f"  {key:<24} {value}")
    print(
        "Network-marked tasks : "
        f"{sum(1 for task in selected if is_network_task(task))}"
    )
    print(
        "Multi-session tasks  : "
        f"{sum(1 for task in selected if task.multi_session)}"
    )
    print(f"Judge needed         : {'yes' if needs_judge else 'no'}")
        ...
        for workspace_file in task.workspace_files:
            if not isinstance(workspace_file, dict):
                fixture_failures.append(
                    f"{task.task_id}: invalid workspace file entry"
                )
                continue
            ...
            if not any(candidate.exists() for candidate in candidates):
                fixture_failures.append(
                    f"{task.task_id}: missing fixture {source}"
                )
    ...
    if gemini_failures:
        print("Gemini environment failures:")
        for item in gemini_failures:
            print(f"  {item}")
    else:
        print("Gemini environment failures: none")
...
def build_turns(task: Task) -> list[dict[str, Any]]:
    ...
    the first item. The first executable turn must always start a fresh Gemini
...
def execute_task(
    ...
    gemini_cli_versions: set[str] = set()
    ...
    for turn_index, turn in enumerate(turns, start=1):
        ...
        # Keep the exact prompt out of Windows .cmd argv. Gemini CLI reads
        ...
        turn_result = run_gemini_streaming(
            command,
            cwd=workspace,
        ...
        if turn_result.get("gemini_cli_version"):
            gemini_cli_versions.add(str(turn_result.get("gemini_cli_version")))
        ...
        turn_results.append({
            ...
            "gemini_cli_version": turn_result.get("gemini_cli_version"),
    ...
    return {
        ...
        "gemini_cli_versions": sorted(gemini_cli_versions),
...
def build_args() -> argparse.Namespace:
    ...
    inferred_home = inferred_root / "gemini-home"

    parser = argparse.ArgumentParser(
        description=(
            "Run local PinchBench tasks with Gemini CLI on native Windows, "
            "one task at a time."
        ),
    ...
    parser.add_argument(
        "--expected-pinchbench-commit",
        default=DEFAULT_PINCHBENCH_COMMIT,
        help="正式比较要求的 PinchBench 精确 git commit",
    )
    parser.add_argument(
        "--expected-manifest-sha256",
        default=DEFAULT_MANIFEST_SHA256,
        help="正式比较要求的 tasks/manifest.yaml SHA-256",
    )
    parser.add_argument(
        "--suite",
        default="core",
        help=(
            "all、core、automated-only、llm-judge-only、hybrid-only、"
            "judge-required-only，或逗号分隔任务 ID"
        ),
    )
    parser.add_argument(
        "--limit",
    ...
    parser.add_argument(
        "--skip",
        default="",
        help="额外跳过的任务 ID；四个 integration 任务默认跳过",
    )
    parser.add_argument(
        "--skip-network",
        action="store_true",
        help="跳过联网任务；正式全量测试通常不应使用",
    )
    parser.add_argument(
        "--gemini-cli",
        default=(
            str(inferred_gemini)
            if inferred_gemini.exists()
            else "auto"
        ),
        help="Gemini CLI 命令、gemini.cmd 或固定绝对路径",
    )
    parser.add_argument(
        "--expected-gemini-version",
        default=DEFAULT_GEMINI_VERSION,
        help="正式测试要求的 Gemini CLI 精确版本",
    )
    parser.add_argument(
        "--gemini-home",
        default=str(inferred_home),
        help="隔离的 GEMINI_CLI_HOME",
    )
    parser.add_argument(
        "--adapter-url",
        default=DEFAULT_ADAPTER_URL,
        help="本机 Gemini-to-OpenRouter adapter URL",
    )
    parser.add_argument(
        "--expected-adapter-version",
        default=DEFAULT_ADAPTER_VERSION,
        help="正式测试要求的 adapter 精确版本",
    )
    parser.add_argument(
        "--model",
        default=DEFAULT_MODEL,
        help="Gemini CLI 模型 ID；adapter 强制映射到该模型",
    )
    parser.add_argument(
        "--approval-mode",
        choices=["default", "auto-edit", "plan", "yolo"],
        default=DEFAULT_APPROVAL_MODE,
        help="正式无人值守测试固定为 yolo",
    )
    parser.add_argument(
        "--timeout-multiplier",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=300.0,
        help="PinchBench LLM Judge 单次请求超时秒数",
    )
    parser.add_argument(
        "--judge-model",
        default=PINCHBENCH_DEFAULT_JUDGE_MODEL,
        help=(
            "传给同一 PinchBench grading code 的 Judge 模型。"
            "只改变模型，不改变 rubric、权重或合成逻辑。"
        ),
    )
    parser.add_argument(
        "--results-dir",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="不追加隔离 workspace 提醒",
    )
    parser.add_argument(
        "--preflight",
        action="store_true",
        help="只做本地预检，不调用 Agent 或 Judge",
    )
    parser.add_argument(
        "--no-judge-cache",
        action="store_true",
    ...
    parser.add_argument(
        "--clear-judge-cache",
        action="store_true",
        help="运行前清空 Gemini 项目的 Judge 缓存",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
def main() -> int:
    ...
    inferred_root = script_path.parent.parent
    gemini_home = Path(args.gemini_home).expanduser().resolve()
    os.environ["GEMINI_CLI_HOME"] = str(gemini_home)
    os.environ["GEMINI_CLI_TRUST_WORKSPACE"] = "true"
    os.environ["GEMINI_API_KEY"] = "AIzaSy" + ("0" * 33)
    os.environ["GOOGLE_GEMINI_BASE_URL"] = args.adapter_url.rstrip("/")
    ...
    gemini_command = choose_gemini_command(args.gemini_cli, inferred_root)
    ...
    if needs_judge and not judge_key_present:
        raise SystemExit(
            ...
            "OPENROUTER_API_KEY。窗口 B 需要该 Key 直接调用 Judge。"
        )
    ...
    run_id = dt.datetime.now().strftime("gemini_%Y%m%d_%H%M%S")
    ...
    print(f"Gemini CLI            : {command_output([gemini_command, '--version'])}")
    print(f"Model                : {args.model}")
    print(f"Approval mode        : {args.approval_mode}")
