...
def translate_tools(gemini_tools: Any) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    def register_call(item: dict[str, Any], fallback: Any = None) -> str:
        ...
        call_state[canonical] = {
            ...
            "arguments": str(
                ...
                else existing.get("arguments") or ""
            ),
    ...
    for raw_line in lines:
        ...
        if event_type == "response.function_call_arguments.done":
            ...
            if isinstance(event.get("arguments"), str):
                call_state[key]["arguments"] = event["arguments"]
            if event.get("name"):
...
class AdapterHandler(BaseHTTPRequestHandler):
    ...
    def begin_sse(self, request_id: str) -> None:
    ...
    def send_sse_buffered(
        ...
        """Send one complete Gemini SSE event with an explicit length.

        Gemini CLI 0.52.0 uses a stream parser that can reject fragmented
        custom-gateway SSE bodies with a residual-buffer error. The earlier
        contract probe proved that one complete data event with Content-Length
        is accepted. OpenRouter is still consumed as a stream internally, but
        Gemini CLI receives one complete event per model turn.
        """
        data = json.dumps(
            payload,
            ensure_ascii=False,
    ...
    def send_sse_comment(self, text: str = "keep-alive") -> None:
    ...
    def do_POST(self) -> None:
        ...
        # Reliability mode for Gemini CLI custom gateways:
        # aggregate the upstream OpenRouter SSE and return one complete Gemini
        # SSE event with Content-Length. Tools, usage, finish reason, and
        # multi-turn history remain intact; only token-by-token display is
        ...
        self.runtime.request_log.write(
            {
                ...
                "response_mode": "buffered_single_sse_event",
                "upstream_headers": safe_headers,
                "upstream_model": aggregate.model,
                "upstream_provider": aggregate.provider,
                "response_id": aggregate.response_id,
        ...
        self.send_sse_buffered(response_payload, request_id)
