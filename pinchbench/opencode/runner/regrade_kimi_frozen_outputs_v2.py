...
REGRADER_REVISION = "2026-08-10-kimi-frozen-regrade-v2"
...
def install_patches(grader: Any, judge_model: str) -> dict[str, Any]:
    patches: dict[str, Any] = {
        ...
        "regrader_revision": REGRADER_REVISION,
    ...
    # automated check hard-codes executable="/bin/bash".  On Windows, merely
    # replacing executable with bash.exe is NOT enough when shell=True because
    # subprocess still uses Windows shell invocation semantics.  Rewrite those
    # two calls to invoke Git Bash explicitly as [bash.exe, "-c", script].
    ...
    def patched_extract(task: Any) -> str:
        ...
        if os.name == "nt" and '/bin/bash' in code:
            ...
            # This official task has two subprocess.run blocks that use
            # shell=True + executable="/bin/bash": one for helper commands and
            # one for executing recovery.sh.  Convert both to direct bash -c.
            code = code.replace(
            ...
            if '/bin/bash' in code:
                raise RuntimeError("Windows Git Bash patch did not replace every /bin/bash occurrence")
        return code
    ...
    def robust_call_judge_api(*, prompt: str, model: str, timeout_seconds: float = 300.0) -> dict[str, Any]:
        ...
        system_msg = (
            ...
            "Keep notes concise (at most about 400 characters)."
        )
        schema = {
            ...
            "schema": {
                ...
                "properties": {
                    ...
                    "notes": {
                        "type": "string",
                        "description": "Concise grading justification, preferably <= 400 characters.",
                    },
                },
        ...
        for structured in (True, False):
            ...
            # OpenRouter can return partial content together with an error, and
            # a completion can be cut at max_completion_tokens.  Never grade a
            # partial JSON object as if it were final.
            if choice_error:
                last_error = f"Judge provider error with partial response: {choice_error}"
                if structured:
                    continue
            ...
            last_error = (
                "Judge response had empty or non-parseable JSON content "
                f"(finish_reason={finish_reason or 'unknown'}, chars={len(text)})"
            )
...
def make_summary(rows: list[dict[str, Any]], original_rows: list[dict[str, Any]], meta: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "Judge结构化输出": True,
        "Judge解析失败重试": True,
        "Judge输出上限": meta["patches"].get("judge_max_completion_tokens"),
        "Regrader版本": REGRADER_REVISION,
...
def main() -> int:
    ...
    out_dir = run_dir / f"regrade_opus5_{args.scope}_v2_{stamp}"
    ...
    log(f"Regrader: {REGRADER_REVISION}")
