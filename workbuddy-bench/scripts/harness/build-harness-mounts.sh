...
mapfile -t ROWS < <(PYTHONPATH="$REPO_ROOT/src${PYTHONPATH:+:$PYTHONPATH}" uv run python - "$REPO_ROOT" "$CONFIGS_DIR" "$HARNESS_FILTER" <<'PY'
...
for row in "${ROWS[@]}"; do
    ...
    mapfile -t ARG_PAIRS < <(uv run python - "$build_args_json" <<'PY'
import json, sys
